<x-app-layout :title=" config('app.name') . ' | ' . 'تعديل تسعيرة' ">
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('itemServices.update', $itemService) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="row mb-4">
                                ...
                                <div class="select2-dark">
                                    <select id="item_id" class="select2 form-select" name="item_id">
                                        ...
                                        @foreach($items as $item)
                                            <option
                                                @selected( old('item_id', $itemService->item_id) == $item->id) value="{{ $item->id }}">{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="select2-dark">
                                    <select id="service_id" class="select2 form-select" name="service_id">
                                        ...
                                        @foreach($services as $service)
                                            <option
                                                @selected( old('service_id', $itemService->service_id) == $service->id) value="{{ $service->id }}">{{ $service->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" class="form-control" id="price"
                                       min="0"
                                       name="price" value="{{ old('price', $itemService->price) }}"/>
                            ...
                            <div class="my-3 mb-md-0 col-md-4">
                                ...
                                <input type="text" class="form-control" id="note" name="note"
                                       placeholder="ملحوظه تظهر في حاله عدم وجود سعر للخدمه"
                                       value="{{ old('note', $itemService->note) }}">
                        ...
                        <div>
                            <button type="submit"
                                    class="btn btn-primary">تعديل تسعيرة
                            </button>
                        </div>
                    </form>
                </div>
