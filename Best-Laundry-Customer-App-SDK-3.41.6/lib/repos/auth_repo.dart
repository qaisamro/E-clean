...
abstract class IAuthRepo {
  ...
  Future<String> forgotPassword({required String contact}); //
  Future<void> logout({
    required String device_key,
  });
...
class AuthRepo extends IAuthRepo {
  ...
  @override
  Future<String> forgotPassword({required String contact}) async {
    ...
    // الـ API يرجع {data:{otp:1234}} - نعرضه داخل التطبيق مجاناً
    try {
      final Map<String, dynamic> body = response.data as Map<String, dynamic>;
      final dynamic d = body['data'];
      if (d is Map && d['otp'] != null) return d['otp'].toString();
      if (body['otp'] != null) return body['otp'].toString();
    } catch (_) {}
    return '';
  }
