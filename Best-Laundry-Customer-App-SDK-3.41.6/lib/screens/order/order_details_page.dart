                ...
                child: ref.watch(orderDetailsProvider(widget.orderID)).map(
                      ...
                      loaded: (_) {
                        Future.delayed(const Duration(milliseconds: 50), () {
                          setState(() {
                            isChatAble =
                                _.data.data!.order!.isChatAble ?? false;
                          });
                        });
                        ...
                        return ListView(
                          ...
                          children: [
                            ...
                            AppSpacerH(40.h),
                            if (_.data.data!.order!.paymentUrl != null &&
                                _.data.data!.order!.paymentType != 'cash')
                              CancelOrderButton(
                                title: 'Pay Now',
                                onTap: () async {
                                  // todo inwebviewpayment url show in webview
                                  final result = await Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                      builder: (context) => WebPayementScreen(
                                        webPaymentScreenAr: WebPaymentScreenArg(
                                          orderId: _.data.data!.order!.id,
                                          paymentUrl:
                                              _.data.data!.order!.paymentUrl!,
                                        ),
                                      ),
                                    ),
                                  );
