...
import 'package:laundry_customer/widgets/custom_dialog.dart';
...
class _WebPayementScreenState extends ConsumerState<WebPayementScreen> {
          ...
          children: [
            InAppWebView(
              ...
              onLoadStart: (controller, url) {
                final String onLoadUrl = url.toString();
                debugPrint("loadurl $onLoadUrl");
                if (onLoadUrl.trim().contains('/payment/success')) {
                  _buildRouting();
                  _buildPaymentDoneDialog();
                } else if (onLoadUrl.contains('payment/pending')) {
                  _buildRouting();
                  _buildPaymentFailedDialog();
                }
                setState(() {
  ...
  Future _buildPaymentFailedDialog() {
    return showDialog(
      context: ContextLess.context,
      builder: (_) => CustomDialog(
