                ...
                <div class="card">
                        ...
                        <div class="row">
                            ...
                            <div class="col-6 position-relative">
                                <div class="position-absolute" style="right: 1em">
                                    <a href="{{ route('coupon.index') }}" class="btn btn-dark"><i
                    ...
                    <div class="card-body">
                        <div class="col-8 m-auto">
                            <form
                                @role('root|admin') @can('coupon.update') action="{{ route('coupon.update', $coupon->id) }}" @endcan @endrole
                                method="POST"> @csrf
                                <div class="row">
                                        ...
                                        <select name="discount_type"
                                            ...
                                            @foreach (config('enums.coupons.discount_types') as $discountType)
                                                <option value="{{ $discountType }}"
                                                    {{ $coupon->discount_type == $discountType ? 'selected' : '' }}>
                                                    {{ __($discountType) }}</option>
                                            @endforeach
                                        </select>
                                        @error('discount_type')
                                        ...
                                        <div class="row">
                                            ...
                                            @can('coupon.update')
                                                <div class="col-12 col-md-6 text-right">
                                                    <button type="submit"
                                                        class="btn btn-primary px-5 @role('visitor') visitorMessage @endrole">{{ __('Update') }}</button>
                                                </div>
                                            @endcan
                                        </div>
                                    </div>
