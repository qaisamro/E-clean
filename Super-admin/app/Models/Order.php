...
class Order extends Model
{
    ...
    // Order.php
    public function getPickDateTimeAttribute()
    {
        if (!$this->pick_date) return null;
        $time = $this->pick_hour ?? '00:00:00';
        return Carbon::parse($this->pick_date->format('Y-m-d') . ' ' . $time);
    }
    public function getDeliveryDateTimeAttribute()
    {
        if (!$this->delivery_date) return null;
        $time = $this->delivery_hour ?? '00:00:00';
        return Carbon::parse($this->delivery_date->format('Y-m-d') . ' ' . $time);
    }
    ...
    public function products()
    {
        return $this->belongsToMany(Product::class, (new OrderProduct())->getTable())
            ->withPivot('quantity', 'price', 'title')
    ...
    public function getPaymentMethodLabelAttribute()
    {
        ...
        if (strtolower($method) == 'visa' || strtolower($method) == 'card') {
            return 'Visa / Card';
        }
        return 'Cash';
    }

    public function getDisplayOrderCodeAttribute()
    {
