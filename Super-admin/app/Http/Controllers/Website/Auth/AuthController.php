...
class AuthController extends Controller
{
    ...
    public function register(RegistrationRequest $request, UserRepository $userRepository)
    {
        ...
        Auth::login($user);
        return redirect()->route('web.home')->with('success', 'تم إنشاء الحساب بنجاح!');
    }
    ...
    public function login(LoginRequest $request)
    {
        ...
        if (Auth::attempt($credentials, $remember)) {
            ...
            if($service_id) {
                return redirect()->route('web.cart', ['service_id' =>  $service_id])->with('success', 'تم تسجيل الدخول بنجاح');
            }

            return redirect()->route('web.home')->with('success', 'تم تسجيل الدخول بنجاح');
        }
