    ...
    <div
        ...
        <div class="space-y-[10px]">
            <p class="text-2xl lg:text-[32px] text-start font-semibold text-white">
                {{ __('Your Care, Our Promise') }} <br class="hidden md:block">{{ __('of') }} <span
                    class="text-mint-600 font-playfair italic">{{ __('Perfection') }}</span>
            </p>
            <p class="text-base lg:text-xl text-left text-neutral-50">
                {{ __('We’re dedicated to quality, hygiene, And') }} <br class="hidden md:block">
                {{ __('On-time delivery — always.') }}
            </p>
        </div>
