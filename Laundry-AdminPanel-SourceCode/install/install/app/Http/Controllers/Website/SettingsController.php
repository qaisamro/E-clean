...
class SettingsController extends Controller
{
    ...
    public function payOrder($orderId)
    {
        ...
        if ($order->payment_status === config('enums.payment_status.paid')) {
            return redirect()->route('web.orders', $order->id)->with('info', 'Order is already paid.');
        }
        ...
        try {
            $paymentUrl = (new PaymentRepository())->stripePaymentURL([
                ...
                'product_data' => "Order #{$order->prefix}-{$order->order_code}",
            ], $order->id);

            return redirect($paymentUrl);
        } catch (\Exception $e) {
            return redirect()->route('web.orders', $order->id)
                ->with('error', 'Unable to initialize Stripe payment: ' . $e->getMessage());
        }
    }
    ...
    public function storeAddress(AddressRequest $request)
    {
        ...
        if ($addressCount >= 3) {
            if ($request->ajax()) {
                return response()->json(['success' => false, 'message' => "Sorry, you can't add more than 3 addresses"]);
            }
            return back()->with('error', 'Sorry, you can\'t add more than 3 addresses');
        }
