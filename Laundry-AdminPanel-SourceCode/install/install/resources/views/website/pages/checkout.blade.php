        ...
        <main class="max-w-6xl mx-auto px-4 py-12 flex flex-col lg:flex-row gap-8">
            ...
            <div class="flex-1 space-y-6">
                ...
                <div class="bg-white p-6 rounded-3xl">
                    ...
                    <p id="no-address-message" class="text-red-500 mb-2 {{ !$addresses->isEmpty() ? 'hidden' : '' }}">Please
                        add a delivery address first.</p>
                    ...
                    <button type="button" class="w-full mt-2 border py-2 rounded-lg text-sm hover:bg-gray-50"
                        onclick="document.getElementById('add-address-modal').classList.remove('hidden')">Add New
                        Address</button>
                </div>

                <!-- Pick-up Schedule -->
                <div class="bg-white p-6 rounded-3xl">
                    <h3 class="font-semibold text-lg mb-4">Pick-up Schedule</h3>
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            <label for="pick_date" class="block text-sm font-medium text-gray-700 mb-2">Pick-up Date</label>
                            <input type="date" name="pick_date" id="pick_date"
                                ...
                                min="{{ date('Y-m-d') }}" value="{{ old('pick_date') }}" required placeholder="Select pick-up date">
                            @error('pick_date')
                        ...
                        <div>
                            <label for="pick_hour" class="block text-sm font-medium text-gray-700 mb-2">Pick-up Time</label>
                    ...
                    <p class="text-sm text-gray-500 mt-4">Choose your preferred pick-up date and time slot.</p>
                </div>

                <!-- Delivery Schedule -->
                <div class="bg-white p-6 rounded-3xl">
                    <h3 class="font-semibold text-lg mb-4">Delivery Schedule</h3>
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            <label for="delivery_date" class="block text-sm font-medium text-gray-700 mb-2">Delivery Date</label>
                            <input type="date" name="delivery_date" id="delivery_date"
                                class="w-full border p-3 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                min="{{ date('Y-m-d') }}" value="{{ old('delivery_date') }}" required placeholder="Select delivery date">
                            @error('delivery_date')
                                <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="delivery_hour" class="block text-sm font-medium text-gray-700 mb-2">Delivery Time</label>
                    ...
                    <p class="text-sm text-gray-500 mt-4">Choose your preferred delivery date and time slot.</p>
                </div>
                ...
                <div class="bg-white p-6 rounded-3xl space-y-4">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="text-sm text-gray-500">{{ count($products) }} items</span>
                    </div>
                            ...
                            <div class="flex justify-between items-center mb-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-sm text-gray-500">{{ $serviceProducts->count() }} items</span>
                                    </div>
                                </div>
                                ...
                                <div class="flex justify-between items-center mb-2 border rounded-lg p-2">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-green-600">{{ currencyPosition(number_format($price, 2)) }} each</p>
                                        </div>
                                    </div>
            ...
            <div class="w-full lg:w-1/3 bg-white p-6 rounded-3xl space-y-4 sticky top-20">
                ...
                <div class="border-t pt-2 space-y-2">
                    <div class="flex justify-between text-sm">
                        <span>Subtotal</span><span>{{ currencyPosition(number_format($subtotal, 2)) }}</span>
                    </div>
                    <div class="flex justify-between text-sm"><span>Delivery Fee</span><span>Free</span></div>
                    <div class="flex justify-between text-sm"><span>Tax & VAT
                ...
                <div class="bg-white p-6 rounded-3xl shadow-sm">
                    <h2 class="text-lg font-semibold text-neutral-700 mb-4">Payment Method</h2>
                ...
                <button type="submit" class="w-full bg-green-600 text-white py-3 rounded-lg font-semibold mt-2"
                    ...
                    Place Order — {{ currencyPosition(number_format($grandTotal, 2)) }}
                </button>
            </div>
        ...
        <div class="bg-white rounded-3xl p-6 w-full max-w-xl mx-4 max-h-[90vh] overflow-y-auto border border-gray-200 shadow-lg">
            ...
            <form id="add-address-form" method="POST" action="{{ route('web.addresses.store') }}">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Road No.</label>
                        <input type="text" name="road_no" class="w-full border p-3 rounded-lg" placeholder="Road #">
                        @error('road_no')
                            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="delivery_note" class="w-full border p-3 rounded-lg" placeholder="Delivery instructions"
                            rows="2"></textarea>
                        @error('delivery_note')
                            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                        @enderror
                ...
                <div class="flex gap-3 mt-4">
                    <button type="button" onclick="document.getElementById('add-address-modal').classList.add('hidden')"
                        class="flex-1 border py-3 rounded-lg hover:bg-gray-50">Cancel</button>
                    <button type="submit" class="flex-1 bg-green-600 text-white py-3 rounded-lg hover:bg-green-700">Save
                        Address</button>
                </div>
            </form>
        </div>
            ...
            fetch(form.action, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        messageDiv.textContent = data.message || 'Address added successfully!';
