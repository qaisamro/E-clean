...
<body>
    <div class="row pt-5 mb-4">
        @php
            $logoPath = public_path('web/logo.png');
            // Try WebSetting logo first
            $webLogo = \App\Models\WebSetting::first()?->websiteLogo;
            if ($webLogo && \Illuminate\Support\Facades\Storage::exists($webLogo->src)) {
                $logoPath = storage_path('app/public/' . $webLogo->src);
            }
        @endphp
        <img class="float-left mr-4 pdf-logo" src="{{ $logoPath }}" alt="">
        <p class="m-0 float-left" style="color:#000 !important">
            Reporte Print Date: {{ now()->format('M d, Y') }}
            <br>
            Reporte printer By: {{ auth()->user()?->name ?? 'System' }}
            <br>
