...
<head>
    ...
    <title>#{{ $order->order_code }} - invioce</title>
</head>
...
<body>
    ...
    <div class="header">
        @php
            $invoiceLogoPath = public_path('web/logo.png');
            $wsLogo = $webSetting?->websiteLogo;
            if ($wsLogo && \Illuminate\Support\Facades\Storage::exists($wsLogo->src)) {
                $invoiceLogoPath = storage_path('app/public/' . $wsLogo->src);
            }
        @endphp
        <div class="logo">
            <img src="{{ $invoiceLogoPath }}" alt="">
        </div>
    </div>
    ...
    <div class="container">
        ...
        <div class="address_line">
            <div class="w-70 float-left">
                ...
                <h4>Payment Method: <strong>{{ $order->payment_method_label }}</strong> | Status:
        ...
        <table class="table">
            ...
            <tbody>
                @foreach ($order->products as $product)
                    ...
                    @endphp
                    <tr>
                        <td>{{ $product->pivot->title ?: $product->name }}</td>
                        <td>{{ $product->pivot->quantity }}</td>
                        <td>{{ currencyPosition($unitPrice) }}</td>
                        <td style="text-align: right">
                            {{ currencyPosition($product->pivot->quantity * $unitPrice) }}
                        </td>
                    </tr>
                @endforeach
                ...
                <tr>
                    ...
                    <td style="text-align: right; background: rgba(0, 0, 0, 0.07);">
                        <strong>{{ currencyPosition($subTotal) }}</strong>
                    </td>
                </tr>
                <tr>
    ...
    <div class="footer">
        ...
        <div class="footer-bottom">
            ...
            @php
                $sigPath = public_path('web/signature.png');
                $sigModel = $webSetting?->signature;
                if ($sigModel && \Illuminate\Support\Facades\Storage::exists($sigModel->src)) {
                    $sigPath = storage_path('app/public/' . $sigModel->src);
                }
            @endphp
